...
void IRGenDebugInfoImpl::addFailureMessageToCurrentLoc(IRBuilder &Builder,
  ...
  if (It != RuntimeErrorFnCache.end())
    ...
    FuncName += failureMsg;
    // CodeView consumers do not correctly handle an artificially generated file,
    // thus use the original location's file as the file for the debug function,
    // and prevent reuse of this debug function. 
    bool useCompilerGeneratedFile = Opts.DebugInfoFormat != IRGenDebugInfoFormat::CodeView;
    llvm::DIFile *File = useCompilerGeneratedFile ? getOrCreateFile({}, {}) : TrapLoc->getFile();
  ...
  // Line 0 is invalid in CodeView, so use the line and column from the original trap location.
  auto DL = llvm::DILocation::get(IGM.getLLVMContext(), Opts.DebugInfoFormat == IRGenDebugInfoFormat::CodeView ? TrapLoc.getLine() : 0, Opts.DebugInfoFormat == IRGenDebugInfoFormat::CodeView ? TrapLoc.getCol() : 0, TrapSP, TrapLoc);
  Builder.SetCurrentDebugLocation(DL);
}
