...
llvm::CallInst *IRBuilder::CreateNonMergeableTrap(IRGenModule &IGM,
  ...
  if (IGM.DebugInfo && 
    ...
    // Line 0 is invalid in CodeView, so create a new location that uses the line and column from the inlined location of the trap, that should correspond to its original source location.
    if (TrapLoc.getLine() == 0 && TrapLoc.getInlinedAt()) {
      auto DL = llvm::DILocation::getDistinct(IGM.getLLVMContext(), 
      TrapLoc.getInlinedAt()->getLine(),
      TrapLoc.getInlinedAt()->getColumn(), 
      TrapLoc.getScope(),
